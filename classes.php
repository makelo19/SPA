    ...
    public function importProcessData($newData) {
        ...
        foreach ($groups as $key => $rows) {
             ...
             foreach ($rows as $row) {
                 ...
                 if ($pk) {
                     if (isset($dataMap[$pk])) {
                         $dataMap[$pk] = $this->mergeCaseInsensitive($dataMap[$pk], $row);
                         $countUpdated++;
                     } else {
                         $dataMap[$pk] = $row;
                         $countInserted++;
        ...
        if (!file_exists($this->configFile)) {
            ...
            $files = [
                ...
                'Base_processos_schema' => [
                    ...
                    'DATA'=>'date', 'STATUS'=>'select', 'Nome_atendente'=>'text'
                ],
            ...
            foreach ($files as $file => $defaultTypes) {
                ...
                if ($file == 'Base_processos_schema') {
                     ...
                     $headers = ['DATA', 'Ocorrencia', 'Status_ocorrencia', 'Nome_atendente', 'Numero_Portabilidade', 'CPF', 'AG', 'Certificado', 'PROPOSTA', 'VALOR DA PORTABILIDADE', 'AUT_PROPOSTA', 'PROPOSTA_2', 'MOTIVO DE CANCELAMENTO', 'STATUS', 'Data Cancelamento', 'OBSERVAÇÃO'];
                } else {
