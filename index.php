...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
 ...
 usort($processos, function($a, $b) use ($sortCol, $desc, $clientMap) {
    ...
    if ($act == 'ajax_render_lembretes_table') {
        ...
        // Sort by Data_Lembrete ASC (default for reminders)
        $res = $db->select($targetFiles, $filters, 1, 100000, 'Data_Lembrete', false); 
        $rows = $res['data'];
        ...
        // Helper to keep filter value
        $getFilterVal = function($k) use ($colFilters) { return isset($colFilters[$k]) ? htmlspecialchars($colFilters[$k]) : ''; };
        
        echo '<td><input class="form-control form-control-sm" value="'.$getFilterVal('Ultima_Alteracao').'" onkeyup="filterLembretesCol(this, \'Ultima_Alteracao\')" placeholder="Filtro..."></td>';
        echo '<td><input class="form-control form-control-sm" value="'.$getFilterVal('Data_Lembrete').'" onkeyup="filterLembretesCol(this, \'Data_Lembrete\')" placeholder="Filtro..."></td>';
        foreach($flaggedFields as $f) {
            echo '<td><input class="form-control form-control-sm" value="'.$getFilterVal($f['key']).'" onkeyup="filterLembretesCol(this, \''.htmlspecialchars($f['key']).'\')" placeholder="Filtro..."></td>';
        }
        echo '<td></td>';
        echo '</tr>';
        echo '</thead><tbody>';
        ...
        foreach($rows as $proc) {
            ...
            echo '<td>' . htmlspecialchars(get_value_ci($proc, 'Data_Lembrete') ?: '') . '</td>';
            ...
            echo '<td><button class="btn btn-sm btn-outline-primary" onclick="loadProcess(\'' . htmlspecialchars($port) . '\')"><i class="fas fa-folder-open"></i></button></td>';
            echo '</tr>';
        }
            ...
            <div class="table-responsive">
                <table class="table table-hover table-custom align-middle" id="dash_table_head">
                    <thead><tr>
                        ...
                        <th class="sortable-header" data-col="VALOR DA PORTABILIDADE" onclick="setDashSort('VALOR DA PORTABILIDADE')" style="cursor:pointer">Valor da Portabilidade <span class="sort-icon"><i class="fas fa-sort text-muted ms-1" style="opacity:0.3"></i></span></th>
