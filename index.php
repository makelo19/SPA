    ...
    if ($act == 'ajax_render_lembretes_table') {
        ...
        $renderFilter = function($key, $label) use ($colFilters) {
             ...
             return '<div class="input-group input-group-sm">
                <input class="form-control" value="'.$val.'" onkeyup="filterLembretesCol(this, \''.htmlspecialchars($key).'\')" placeholder="Filtro...">
                <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown"><i class="fas fa-sort"></i></button>
                <ul class="dropdown-menu dropdown-menu-end">
                    <li><h6 class="dropdown-header">Ordenar</h6></li>
                    <li><a class="dropdown-item" href="#" onclick="sortLembretes(\''.htmlspecialchars($key).'\', \'asc\')"><i class="fas fa-sort-amount-down-alt me-2"></i>Antiga/Menor/A-Z</a></li>
                    <li><a class="dropdown-item" href="#" onclick="sortLembretes(\''.htmlspecialchars($key).'\', \'desc\')"><i class="fas fa-sort-amount-down me-2"></i>Recente/Maior/Z-A</a></li>
                </ul>
             </div>';
        };
        ...
        foreach($flaggedFields as $f) {
            $lbl = ($f['key'] == 'Numero_Portabilidade') ? 'Portabilidade' : $f['label'];
        ...
        echo '<td>' . $renderFilter('Ultima_Alteracao', 'Ultima Atualização') . '</td>';
        echo '<td>' . $renderFilter('Data_Lembrete', 'Data Lembrete') . '</td>';
        foreach($flaggedFields as $f) {
        ...
        foreach($rows as $proc) {
            ...
            if ($dlVal) {
                ...
                if (!$dtDL) $dtDL = DateTime::createFromFormat('d/m/Y H:i:s', $dlVal);
...
<script>
    ...
    function sortLembretes(col, dir) {
        lembretesSortCol = col;
        lembretesSortDir = dir;
