...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
 ...
 usort($processos, function($a, $b) use ($sortCol, $desc, $clientMap) {
        ...
        foreach($processos as $proc) {
            ...
            echo '</td>';
            echo '<td class="dashboard-text">' . htmlspecialchars(get_value_ci($proc, 'Data_Ultima_Cobranca') ?: '') . '</td>';
            ...
            <div class="table-responsive">
                <table class="table table-hover table-custom align-middle" id="dash_table_head">
                    <thead><tr>
                        ...
                        <th class="sortable-header" data-col="Data_Ultima_Cobranca" onclick="setDashSort('Data_Ultima_Cobranca')" style="cursor:pointer">Data da Última Cobrança <span class="sort-icon"><i class="fas fa-sort text-muted ms-1" style="opacity:0.3"></i></span></th>
                            ...
                            foreach($procFields as $f): 
                                ...
                                $val = $getVal($processo, $f['key']);
                                    ...
                                    <div class="col-md-4 <?= $hideClass ?>" data-field-key="<?= $f['key'] ?>">
                                        ...
                                        <?php else: ?>
                                            <input type="<?= $f['type']=='date'?'date':'text' ?>" name="<?= $f['key'] ?>" id="proc_<?= $f['key'] ?>" class="form-control form-control-custom" value="<?= htmlspecialchars($val) ?>" <?= $req ?> <?= $disabled ?>>
                                        <?php endif; ?>
                                    </div>
                                    <?php endforeach; ?>
                                    <?php 
